...
#     os.environ.get("MAX_CONTENT_LENGTH", 20 * 1024 * 1024)
# )  # 20 MB
...
if MODEL_NAME == "basic":
    ...
    MODEL_PATH = "models/mobilenetv2.h5"  # fixed: was "mobileNetV2.h5" (case mismatch)
    IMG_SIZE = (128, 128)
    PREPROC = mv2_pre
...
# Load the real class mapping written by the training scripts, instead of
# hardcoding it. This stays correct even if folder/class order ever changes.
class_map = load_class_index("models/class_index.json")

# Build the embedding model ONCE here, not per-request. predict_image_with_ood
# accepts it as an argument so it's reused across every prediction call.
...
def predict():
    ...
    # 2) Embedding metrics for OOD (reuses the embed_model built at startup)
