...
def predict_image_voted(
    ...
    # Stack all crops into a SINGLE batch instead of calling model.predict()
    # once per crop. On CPU especially, each separate predict() call carries
    # fixed dispatch overhead - batching turns 6 slow calls into 1 fast one.
    xs = [_prepare_x_from_pil(c, img_size, preproc)[0] for c in crops]
